 ...
 */
function genji(entries, outFileName, { standalone = '', project = '.', sourceMaps = false, minify = false, watch = false, expose = [], external = [] } = {}) {
    return fs_utils_1.mkdir(path.dirname(outFileName)).then(() => {
        ...
        if (Array.isArray(expose) && expose.length) {
            if (expose.length === entries.length) {
                ...
                return Promise.reject(Object.assign(new Error('Expose must be explicitly specified for each entry file'), { code: 'EINVALIDEXPOSE' }));
            }
        }
    ...
    return new Promise((resolve, reject) => {
        ...
        return Promise.all([
            ...
            sourceMaps && new Promise((resolve, reject) => {
                fs.writeFile(fileName + '.map', map.toString(), 'utf8', error => error ? reject(error) : resolve());
            })
        ]);
    });
}
